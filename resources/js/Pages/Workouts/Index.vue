...
// ─── Ativas / Arquivadas tabs ───────────────────────────────────────────────
const activeTab = ref('active'); // 'active' | 'archived'

const activeWorkouts = computed(() => props.workouts.filter((w) => w.is_active));
const archivedWorkouts = computed(() => props.workouts.filter((w) => !w.is_active));
const visibleWorkouts = computed(() =>
    activeTab.value === 'active' ? activeWorkouts.value : archivedWorkouts.value,
);

const hasWorkouts = computed(() => visibleWorkouts.value.length > 0);
        ...
        <div class="py-6 sm:py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="mb-5 flex gap-2">
                    ...
                    >
                        Fichas Ativas ({{ activeWorkouts.length }})
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        Arquivadas ({{ archivedWorkouts.length }})
                    </button>
                </div>

                <div
                    v-if="!hasWorkouts && activeTab === 'active'"
                ...
                >
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        Nenhuma ficha arquivada no momento.
                    </p>
                </div>
